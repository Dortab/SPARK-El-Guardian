...
const apiProblemUrls = {
    ...
    'mision-divisiones-page': 'https://n8n-spark2.onrender.com/webhook/9f503e35-5bdb-4997-99d4-fa1807b13536' // URL simulada para divisiones
};
...
const apiValidationUrls = {
    ...
    'mision-divisiones-page': 'https://n8n-spark2.onrender.com/webhook/be06672c-bbc0-4180-b456-3fb7f9018232' // URL simulada para validación de divisiones
};
...
function showSparkAlert(title, message, type, callback = null) {
    ...
    // Almacena la función de callback
    alertCallback = callback;

    // Limpia clases anteriores y añade la nueva
    sparkAlertContent.classList.remove('correct', 'incorrect');
    sparkAlertIcon.innerHTML = ''; // Limpia el icono anterior

    if (type === 'correct') {
        sparkAlertContent.classList.add('correct');
        sparkAlertIcon.innerHTML = '&#10003;'; // Tick mark
    } else if (type === 'incorrect') {
        sparkAlertContent.classList.add('incorrect');
        sparkAlertIcon.innerHTML = '&#10007;'; // Cross mark
    }

    sparkAlertContainer.classList.add('active');
}

// Event listener para cerrar la alerta
if (sparkAlertCloseBtn) {
    sparkAlertCloseBtn.addEventListener('click', () => {
        sparkAlertContainer.classList.remove('active');
        // Si hay una función de callback, la ejecuta
        if (alertCallback) {
            alertCallback();
            alertCallback = null; // Limpia el callback para evitar ejecuciones futuras no deseadas
        }
    });
}
...
async function validateAnswer() {
    ...
    try {
        ...
        if (data.result === 'correcta') {
            // Pasa la función loadNewProblem como callback
